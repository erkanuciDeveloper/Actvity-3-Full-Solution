...
def get_input_data()->pd.DataFrame:
    ...
    concatenated_values = df[Config.CLASS_COL].apply(lambda row: ', '.join(row.dropna()), axis=1)

    # Create a new DataFrame with the concatenated values
    new_df = pd.DataFrame({'y': concatenated_values})

    # Concatenate the new DataFrame with the original DataFrame
    df = pd.concat([df, new_df], axis=1)

    #df["y"] = df[Config.CLASS_COL].apply(lambda row: ', '.join(row.dropna()), axis=1)
    df = df.loc[(df["y"] != '') & (~df["y"].isna()),]
    return df


def get_input_data_hierarchical()->pd.DataFrame:
    df1 = pd.read_csv("Actvity 3 Full Solution/data/AppGallery.csv", skipinitialspace=True)
    #df1.rename(columns={'Type 1': 'y1', 'Type 2': 'y2', 'Type 3': 'y3', 'Type 4': 'y4'}, inplace=True)
    df2 = pd.read_csv("Actvity 3 Full Solution/data/Purchasing.csv", skipinitialspace=True)
    #df2.rename(columns={'Type 1': 'y1', 'Type 2': 'y2', 'Type 3': 'y3', 'Type 4': 'y4'}, inplace=True)
    ...
    df[Config.INTERACTION_CONTENT] = df[Config.INTERACTION_CONTENT].values.astype('U')
    df[Config.TICKET_SUMMARY] = df[Config.TICKET_SUMMARY].values.astype('U')
    #df["y"] = df[Config.CLASS_COL_HIERARCHICAL]
...
def de_duplication(data: pd.DataFrame):
    ...
    data.to_csv('Actvity 3 Full Solution/data/out.csv')
    data[Config.INTERACTION_CONTENT] = data['ic_deduplicated']
    data = data.drop(columns=['ic_deduplicated'])
    return data


def de_duplication_hierarchical(data: pd.DataFrame):
    ...
    data.to_csv('Actvity 3 Full Solution/data/out_hierarchical.csv')
    data[Config.INTERACTION_CONTENT] = data['ic_deduplicated']
    data = data.drop(columns=['ic_deduplicated'])
    return data
